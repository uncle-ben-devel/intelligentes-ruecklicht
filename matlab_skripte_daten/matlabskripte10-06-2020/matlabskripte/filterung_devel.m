...
%     accel_raw = accel_raw - mean(accel_raw(1:10));
...
%     filter_absolute_abweichung = absolute_abweichung(accel_raw);
    ...
    extra_stufen=2;     % 2 @100Hz scheint gut
    alpha = 0.1;        % 0.1 @100Hz
...
%% Bremsen feststellen
brems_est_vec = wird_gebremst(filter_exp_moving_avg0, 0, filter_exp_moving_avg0, -0.5);
%brems_est_vec = hysterese(brems_est_vec, 0.05, 0.15, f_sample);

%% Plotten
    plot_compare(accel_raw, filter_exp_moving_avg0, filter_relative_abweichung, zeros(length(tacho_vec),1), brems_vec, t_vec);
