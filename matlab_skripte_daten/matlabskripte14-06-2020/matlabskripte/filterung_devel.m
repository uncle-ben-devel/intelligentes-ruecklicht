...
%     filter_sgolay = sgolayfilt(accel_raw,1,31);
    ...
    %% Filtertest
    filter_test0 = exp_ma_zweistufig(accel_raw,0.04, 0.1);
    filter_test1 = dc_block(filter_test0,alpha_dc_block);
...
%% Bremsen feststellen
brems_est_vec = wird_gebremst(ohnedc1, -0.5, ohnedc1, -0.65);
%   brems_est_vec = hysterese(brems_est_vec, 0.05, 0.15, f_sample);

%% Plotten
%      plot_compare(accel_raw, accel_raw-filter_dc_block, accel_raw-filter_dc_block, brems_vec, brems_vec, t_vec, fahrtinfo);
...
%      plot_all(x_s+40, y_s, z_s-40, brems_vec, brems_vec, t_vec, fahrtinfo);
     plot_all(x_vec+40, y_vec, z_vec-40, brems_vec, brems_vec, t_vec, fahrtinfo);
...
function [] = plot_compare(raw, filter0, filter1, tacho_vec, brems_vec, t_vec, fahrtinfo)
       figure
       hold on
            plx = stairs(t_vec,raw,  'r');
            ply = stairs(t_vec,filter0,  'k');
