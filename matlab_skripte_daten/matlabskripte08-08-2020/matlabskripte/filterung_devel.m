...
%     ohnedc1 = exp_moving_avg(ohnedc0, 0.1);
    ...
    mean_filter = exp_moving_avg(accel_raw, 0.04);
    offset_filter = exp_moving_avg(accel_raw, 0.01);
...
% Schwelle = (-0.15 - ((abweich_filter) .* 0.07)) + exp_moving_avg(accel_raw, 0.01);
% Schwelle = -0.65*ones(length(accel_raw),1);
% brems_est_vec0 = (0 < 1);
% brems_est_vec1 = (mean_filter < Schwelle);
% brems_est_vec = -12*(brems_est_vec0 & brems_est_vec1);
% hyst_filter = hysterese(brems_est_vec, 0.2, 0.05, f_sample);
schwellwert_normal = -0.25 - 0.07 * abweich_filter + offset_filter;
schwellwert_gefahr = -1.1 - 0.07 * abweich_filter + offset_filter;

brems_est_normal = (-6) * (mean_filter < schwellwert_normal);
brems_est_gefahr = (-6) * (mean_filter < schwellwert_gefahr);

hyst_normal = 2 * hysterese(brems_est_normal, 0.05, 0.15, f_sample);
hyst_gefahr = 2 * hysterese(brems_est_gefahr, 0.01, 0.5, f_sample);
%% Plotten
%      plot_compare(accel_raw, mean_filter, hyst_filter, brems_est_vec , brems_vec, t_vec, fahrtinfo);
%      plot_verarbeitungskette(accel_raw, offset_filter, mean_filter, schwellwert_normal, schwellwert_gefahr, brems_est_normal, hyst_normal, brems_est_gefahr, hyst_gefahr, brems_vec, t_vec, fahrtinfo)
...
%% Zeug fuer Doku
%% Filtervergleich SMA EMA
skala = 10^5;
f_sample = 100;
filterbreite = 100;
alpha = 2 / (filterbreite+1);

% Spektrum von Rauschen
noise_100Hz = randn(6*skala,1);
[noise_amplitude, frq] = pwelch(noise_100Hz,[],[],[],f_sample, 'psd');
noise_db=10*log10(noise_amplitude);

% Spektrum SMA
filter_sma = mittelwert(noise_100Hz,filterbreite);
[sma_amplitude, frq] = pwelch(filter_sma,[],[],[],f_sample, 'psd');
sma_db=10*log10(sma_amplitude);

% Spektrum EMA
filter_ema = exp_moving_avg(noise_100Hz, alpha);
[ema_amplitude, frq] = pwelch(filter_ema,[],[],[],f_sample, 'psd');
ema_db=10*log10(ema_amplitude);

figure
hold on
plot(frq, noise_db-mean(noise_db))
plot(frq, ema_db-mean(noise_db), 'k')
plot(frq, sma_db-mean(noise_db),'m')
plot(frq, -3*ones(length(frq),1), 'r.')
hold off
grid on
title("Amplitudenverhalten verschiedener Filter im Vergleich")
xlabel("Frequenz in Hz")
ylabel("Leistung in dB")
legend("Rauschen", ...
    strcat("EMA mit alpha 1 / (1+",int2str(filterbreite), ") bzw. ",num2str(alpha)), ...
    strcat("SMA mit Filterbreite ", int2str(filterbreite)), ...
    "-3dB-Linie")

%% Custom Plot fuer Doku
% 	hold on
%     stairs(t_vec, accel_raw, 'Color', [0.8500, 0.3250, 0.0980])
%     stairs(t_vec,brems_vec, 'c.-');
% 	xlabel("Zeit in s")
% 	ylabel("Beschleunigung in m/s^2")
% 	title("Rohe Beschleunigungswerte und Bremshebelinformation")
%     legend("Rohe Beschleunigungswerte","Bremshebelinformation, HIGH: Bremshebel betätigt")
% 	grid on
    
%     hold on
%     stairs(t_vec, mean_filter, 'Color', [0.8500, 0.3250, 0.0980])
%     stairs(t_vec,brems_vec / 12 * 3, 'c.-');
% 	xlabel("Zeit in s")
% 	ylabel("Beschleunigung in m/s^2")
% 	title("Gefilterte Beschleunigungswerte und Bremshebelinformation")
%     legend("Gefilterte Beschleunigungswerte","Bremshebelinformation, HIGH: Bremshebel betätigt")
% 	grid on

%     hold on
%     stairs(t_vec, hyst_normal / 12, 'Color', [0.8500, 0.3250, 0.0980])
%     stairs(t_vec,brems_vec / 12, 'c.-');
% 	xlabel("Zeit in s")
% 	ylabel("Wahr (1, -1), Falsch (0)")
% 	title("Bremsvermutung und Bremshebelinformation")
%     legend("Bremsvermutung","Bremshebelinformation, HIGH: Bremshebel betätigt")
% 	grid on
    
%     hold on
%     stairs(t_vec, hyst_gefahr / 12, 'Color', [0.8500, 0.3250, 0.0980])
%     stairs(t_vec,brems_vec / 12, 'c.-');
% 	xlabel("Zeit in s")
% 	ylabel("Beschleunigung in m/s^2")
% 	title("Gefahrbremsvermutung und Bremshebelinformation")
%     legend("Gefahrbremsvermutung","Bremshebelinformation, HIGH: Bremshebel betätigt")
% 	grid on
...
function [] = plot_verarbeitungskette(accel_raw, offsetfilter, signalfilter, schwellwert_normal, schwellwert_gefahr, brems_est_normal, hyst_normal, brems_est_gefahr, hyst_gefahr, brems_vec, t_vec, fahrtinfo)
        figure
        hold on
            plx=plot(t_vec,accel_raw,'Color',[0, 0.4470, 0.7410]);
            stairs(t_vec,offsetfilter, 'c*-');
            stairs(t_vec,signalfilter, 'b');
            stairs(t_vec,schwellwert_normal, 'Color',[0.5, 0.5, 0.5]);
            stairs(t_vec,schwellwert_gefahr, 'k');
            stairs(t_vec,brems_est_normal, 'Color', [0.8500, 0.3250, 0.0980]);
            stairs(t_vec,hyst_normal, 'Color', [0.8500, 0.3250, 0.0980]);
            stairs(t_vec,brems_est_gefahr, 'r');
            stairs(t_vec,hyst_gefahr, 'r.-');
            stairs(t_vec,brems_vec, 'm.-');
        hold off
       ...
       legend(  "Rohdaten aus Accelerometer: Beschleunigung auf Bewegungsachse des Rades in m/s^2", ...
                "Gleichanteil des Rohsignals in m/s^2", ...
                "Gefiltertes Signal in m/s^2", ...
                "Schwellwert für normalen Bremsvorgang in m/s^2", ...
                "Schwellwert für Gefahrbremsung in m/s^2", ...
                "Bremsvermutung für normalen Bremsvorgang ja:high, nein:low", ...
                "Bremsvermutung für normalen Bremsvorgang nach Hysterese, tiefer", ...
                "Bremsvermutung für Gefahrbremsung ja:high, nein:low", ...
                "Bremsvermutung für Gefahrbremsung nach Hysterese, tiefer", ...
                "Bremshebel betaetigt ja:high, nein:low" ...
                );
...
function [y] = lokale_abweichung(x, filterbreite)
    ...
    for ii=filterbreite+2:1:length(x)
        ...
        y(ii) = y(ii) - abs(x(ii-filterbreite+1)-x(ii-filterbreite)); %abziehen vom Wert ausserhalb der Filterbreite
    end
    y = y/(filterbreite-1);
end
...
% Funktion, die ueber die Filterbreite die absolute Abweichung (Betrag) zum jeweils naechsten
% Wert aufsummiert und in y zurueckgibt, mit Puffer, damit es wie auf dem
% ESP ist
% function [y] = lokale_abweichung_puffer(x, filterbreite)
%     y = zeros(length(x),1);
%     puffer = zeros(length(filterbreite)+1,1);
%     pufferindex = 0;
%     for ii=filterbreite+2:1:length(x)
%         pufferindex = mod(ii,filterbreite)+1;
%         puffer(pufferindex) = x(ii);
%         xplus= abs(puffer(pufferindex)-puffer(mod(pufferindex + filterbreite -1, filterbreite)+1));
%         xminus= abs(puffer(mod(pufferindex + 2, filterbreite)+1) - puffer(mod(pufferindex + 1, filterbreite)+1));
%         
%         y(ii) = y(ii-1) - xminus + xplus;
%     end
%     y = y/filterbreite;
% end