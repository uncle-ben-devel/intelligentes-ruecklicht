...
void setup() {                                        // put your setup code here, to run once:
  ...
  interrupts();
attachInterrupt(digitalPinToInterrupt(TACHO),ISR_Tacho, RISING); //digitalPinToInterrupt
}
...
float FilterBuffer(float* Buffer){
 ...
 void Ausgabe(void){
  ...
  //Ausgabe für Plotter
    if (Ausgabeformat == 2){
      Serial.println("Geschwindigkeit_in_m/s Beschleunigung_in_m/s^2 Gefilterte_Beschleunigungswerte_in_m/s^2"); //legend for Serial Plotter
      Serial.print(GeschwindigkeitNeu, 20);
      Serial.print("\t");
      Serial.print(BeschleunigungTacho, 20);
      Serial.print("\t");
      Serial.println(KannWegTacho, 20);
    }
}
