...
//Variablen fuer dynamischen Schwellwert-Filter
#define SizeDSFilter 25
#define Schwellwert_Offset -0.15
#define Schwellwert_Faktor -0.07
byte BufferDSIndex = 0;
float RingBufferDS [SizeDSFilter + 1] = {0};
float x_plus = 0, x_minus = 0, x_sum = 0; //Summenglieder
float yDS = 0;  //DS Filter Ausgang

//Variablen fuer DC Blocker
#define alphaDCB 0.9999
float xDCB_old = 0;
float yDCB = 0;
...
//EMA Filter zum Glaetten
#define smoothing_alpha0 0.1
#define smoothing_beta0 0.9
#define smoothing_alpha1 0.04
#define smoothing_beta1 0.96
float smoothing_ema_y0 = 0.0;
float smoothing_ema_y1 = 0.0;

//wieder weg:
float ema_ynew = 0;

//Schwellwert
float Schwellwert = -0.60;
...
//  }
  ...
  //after Timer ISR
   if(Timer1Flag){ //nach Timerueberlauf alle 5ms
    bno.getEvent(&AccelData, Adafruit_BNO055::VECTOR_ACCELEROMETER);
    //bno.getEvent(&AccelData, Adafruit_BNO055::VECTOR_LINEARACCEL);
    Serial.println("x_raw,x_new(ohne_Offset),ema_ynew,Schwellwert,Bremsvermutung,licht_an"); //legende fuer Plotter
    x_raw = -1.0 * (AccelData.acceleration.x);
    Serial.print(x_raw);
    Serial.print("  ");
    //cumulative_avg();
    //x_new = x_raw - cma_ynew;
    //DC_Block(); //entfernen des DC Anteils
    //float EMA(float* x, float* y float alpha, float beta)
    //x_new = EMA(&x_raw, &yEMA1, 0.01, 0.99);
    x_new = x_raw;
    Serial.print(x_new);
    Serial.print("  ");
    //exp_moving_avg_cascade(); //Kaskadierter EMA Filter fuer Signal
    
    //float EMAx2(float* x, float* y0, float* y1, float alpha0, float beta0, float alpha1, float beta1){
    ema_ynew = EMAx2(&x_new, &smoothing_ema_y0, &smoothing_ema_y1, smoothing_alpha0, smoothing_beta0, smoothing_alpha1, smoothing_beta1);
    Serial.print(ema_ynew);
    Serial.print("  ");
    Schwellwert = Schwellwert_Offset + (Schwellwert_Faktor * dynamischerSchwellwert(&x_raw)); //Schwellwert = A(neg) + B(neg)*(Struung)
    Serial.print(Schwellwert*10);
    Serial.print("  ");
    Bremsvermutung = (ema_ynew < Schwellwert);  //Bremsschwelle ueberschritten?   
    Serial.print(((int)Bremsvermutung)*3);
    Serial.print("  ");
    //bool hysterese(bool* Eingang, bool* Zustand)
    licht_an = hysterese(&Bremsvermutung, &licht_an);
//    Serial.print(t_run);
//    Serial.print("  ");    
//    Serial.print(t_stop);
//    Serial.print("  ");
...
float dynamischerSchwellwert(float* x){
  RingBufferDS[BufferDSIndex] = *x;  //x_raw aus Timer ISR Flag function
  x_plus = RingBufferDS[BufferDSIndex] - RingBufferDS[(BufferDSIndex + SizeDSFilter) % (SizeDSFilter + 1)];
  x_plus = abs(x_plus);
  x_minus = RingBufferDS[(BufferDSIndex + 2) % (SizeDSFilter + 1)] - RingBufferDS[(BufferDSIndex + 1) % (SizeDSFilter + 1)];
  x_minus = abs(x_minus);
  x_sum = x_sum - x_minus + x_plus;
  yDS = x_sum / (SizeDSFilter - 1);
  BufferDSIndex = (BufferDSIndex + 1) % (SizeDSFilter + 1);
  return yDS;
}

void DC_Block(){
  //Entfernen des Gleichanteils
  yDCB = yDCB * alphaDCB + x_raw - xDCB_old;
  xDCB_old = x_raw;
}
...
float EMAx2(float* x, float* y0, float* y1, float alpha0, float beta0, float alpha1, float beta1){
  //Berechnen des Exponental Moving Average Filters
...
float EMA(float* x, float* y, float alpha, float beta){
  //Berechnen des einstufigen Exponental Moving Averrage Filters
  *y = (alpha * *x) + (beta * *y);
  return *y;
}
...
bool hysterese(bool* Eingang, bool* Zustand){
  ...
  if (*Eingang){ t_run = t_run + t_sample;    t_stop = 0.0;
  }else {       t_stop = t_stop + t_sample;  t_run = 0.0;}

  //Ausgang der Hysterese
  if (*Zustand && (t_stop >= t_off)) {   *Zustand = false;
  }else if(!(*Zustand) && (t_run >= t_on)){ *Zustand = true;}
