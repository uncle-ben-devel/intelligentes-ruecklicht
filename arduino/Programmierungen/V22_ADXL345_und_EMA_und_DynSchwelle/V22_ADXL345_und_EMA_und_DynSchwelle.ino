...
#define Ausgabeformat 1

#include <Wire.h>
#include <Ticker.h> //Timer
//Sensor Specific
#include <Adafruit_BNO055.h> //braucht man trotz ADXL! fuer Mathematikgedoens
...
// Variablen fuer die Verarbeitungskette
	...
	#define offset_normal	-0.25	// Gleichanteil der Schwelle
	#define offset_gefahr	-1.1	
...
//######OBJEKTE####
	...
	EMA ema_signal(0.02);	// alpha-wert; EMA fuer die Filterung des Signals
	...
	lichtsteuerung lichtsteuerung_obj(15, f_sample);	// Blinkfrequenz, Samplefrequenz
...
void setup() {  
  ...
  // Auskommentieren fuer Debug ohne Sensor
   ...
   lichtsteuerung_obj.rainbowFade2White(3, 3, 2, 1); //speed RainbowChase, rotations RainbowChase, speed WhiteToRed Fade, count of WhiteToRed pulses
}
//########################################################
//################## MAIN ################################
//########################################################
...
void loop() {
  //after Timer ISR
	if(Timer1Flag){ //nach Timerueberlauf alle t_sample (1 / f_sample)
		...
		// Rohdaten einlesen
			x_raw =eventADXL.acceleration.z;
      ...
      lichtsteuerung_obj.lichtsteuerung_ausfuehren(licht_heller, licht_blink);
			//digitalWrite(LED_BRAKE, lichtsteuerung_obj.lichtsteuerung_ausfuehren(licht_heller, licht_blink));

    //Timer Flag zuruecksetzen.
      Timer1Flag = false;
	}
}
//########################################################
...
void print_data(){
	if (Ausgabeformat == 1){ //Serieller Monitor
	  Serial.print("\n Zeitdifferenz: ");
    Serial.print(millis() - Zeitmerker);
    Zeitmerker = millis();

	};
	
	if (Ausgabeformat == 2){  //Serieller Plotter
  
    //ACHTUNG NICHT 100Hz FAEHIG, Ausgabe dauert etwa 18ms!!!!!!!!!!!!!!!!!!!!!!!!!!!
    
	  Serial.println("Rohdaten, Gleichanteil, tiefpassgefiltert, Streuung_aktuell, Schwellwert_normal, Schwellwert_gefahr, Bremsvermutung_normal, Bremsvermutung_gefahr, Licht_heller, Licht_blink"); //Legende fuer Plotter
  						Serial.print(x_raw); 
