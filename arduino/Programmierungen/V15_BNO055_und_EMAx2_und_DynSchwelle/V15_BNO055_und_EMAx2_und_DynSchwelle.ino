...
//Dynamischer Schwellwert-Filter anwenden
...
//  }
  ...
  //after Timer ISR
   if(Timer1Flag){ //nach Timerueberlauf alle 5ms
    bno.getEvent(&AccelData, Adafruit_BNO055::VECTOR_ACCELEROMETER);
    //bno.getEvent(&AccelData, Adafruit_BNO055::VECTOR_LINEARACCEL);
    Serial.println("x_new(ohne_Offset),ema_ynew,Schwellwert,Bremsvermutung,t_run,t_stop,licht_an"); //legende fuer Plotter
    ...
    Serial.print("  ");
    dynSchwellwert();
    Schwellwert = Schwellwert_Offset + (Schwellwert_Faktor * yDS); //Schwellwert = A(neg) + B(neg)*(Struung)
...
void dynSchwellwert(){
  RingBufferDS[BufferDSIndex] = x_raw;  //x_raw aus Timer ISR Flag function
...
void exp_moving_avg_cascade(){
  //Berechnen des Exponental Moving Averrage Filters
  ema_ynew0 = (alpha0 * x_new) + (beta0 * ema_yold0);
  ema_yold0 = ema_ynew0;
  //zweite Stufe
  ema_ynew1 = (alpha1 * ema_ynew0) + (beta1 * ema_yold1);
  ema_yold1 = ema_ynew1;
  ema_ynew = ema_ynew1;
}
