...
typedef struct MessElement {float x_AccelMess; float y_AccelMess; float z_AccelMess; byte Brake_Mess;};

#define SpeicherArraySize 3200  //max. 5000, dann Speicher voll
...
void setup() {                                      
  ...
  timer1_write(50000);  // 50000 / (5ticks per us from TIM_DIV16) = 10.000 us interval =100Hz
...
void loop() {
  ...
  //after Timer ISR
 if(Timer1Flag && DataDumpPinMerker){ //nach Timerueberlauf alle 5ms
  ...
  if(ii < SpeicherArraySize) {
    ...
    if(!ArrayFullFlag){
      ...
      digitalWrite(LED_ONBOARD,HIGH); //lowaktiv
      Serial.print("MessArray voll, Interrupts deaktiviert. millis:");
      Serial.println(millis());
      ArrayFullFlag = true;
    }
  };
 ...
 if(digitalRead(DataDumpPin) == LOW){
  if(DataDumpPinMerker){
        ...
        Serial.print("Starte Messvorgang in 1 Sekunde.  millis:");
        interrupts();
...
void SaveMessurementToArray(){
  ...
  SpeicherArray[ii].x_AccelMess = eventADXL.acceleration.x;
  SpeicherArray[ii].y_AccelMess = eventADXL.acceleration.y;
  SpeicherArray[ii].z_AccelMess = eventADXL.acceleration.z;
...
//      Serial.print("\t");
      Serial.print(SpeicherArray[aa].x_AccelMess, 20);
      Serial.print("\t");
      Serial.print(SpeicherArray[aa].y_AccelMess, 20);
      Serial.print("\t");
      Serial.print(SpeicherArray[aa].z_AccelMess, 20);
      Serial.print("\t");
...
//
//      //Beschleunigungen in m/s^2, später ggf. die Rohwerte verarbeiten, sodass man sich die Umrechnung schenken kann
//      Serial.print(AccelData.acceleration.x, 20);
//      Serial.print("\t");
